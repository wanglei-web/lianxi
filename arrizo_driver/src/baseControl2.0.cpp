...
public:
	...
	void vehicleCtrlCmd1_callback(const arrizo_msgs::VehicleCtrlCmd::ConstPtr msg);
    void vehicleCtrlCmd2_callback(const arrizo_msgs::VehicleState::ConstPtr msg);//
...
private:
	...
	ros::Subscriber vehicleCtrlCmd1_sub_;
    ros::Subscriber vehicleCtrlCmd2_sub_;
...
bool BaseControl::init(int argc,char**argv)
{
	...
	vehicleCtrlCmd1_sub_ = nh.subscribe("/VehicleCtrlCmd",1,&BaseControl::vehicleCtrlCmd1_callback,this);
    vehicleCtrlCmd2_sub_ = nh.subscribe("/VehicleCtrlState",1,&BaseControl::vehicleCtrlCmd2_callback,this);
...
void BaseControl::vehicleCtrlCmd1_callback(const arrizo_msgs::VehicleCtrlCmd::ConstPtr msg)
{
   ...
   float set_speed  =(msg->set_speed)*10;
	...
	g_cmdspeed = msg->set_speed; 
    g_cmdangle = msg->set_roadWheelAngle;
	g_gear = set_gear;
...
void BaseControl::vehicleCtrlCmd2_callback(const arrizo_msgs::ControlState::ConstPtr msg)
{
...
printf("模式：%d / %d \n",set_driverlessMode);
	/*b = (uint16_t(set_driverlessMode) << 4) & 0x30;
	cout << canMsg_cmd1_.data[0] << ","<< endl;
        cout << b << endl;*/ // 验证字节有效性。
